...
class DBManager:
    ...
    A class that interacts with the TinyDB database.
        ...
        -------
            ...
            of a record identified by its doc_id (record_id) in a specific
            table (table_name)
            with a new value (data_value) by replacing the existing value (
            append=False)
            or appending the existing value (append=True)
            get_record_data -> fetches all data (data_key=None) of a record
            identified by
    ...
    def add_record(self, table_name, record_data):
        ...
        Adds a record to a table of a DBManager object
            ...
            Parameters
            ----------
                table_name -> name of the TinyDB table to which the record
                must be added
    ...
    def update_record_data(self, table_name, record_id, data_key, new_value,
                           append=False):
        """
        Updates a record of a DBManager object
            ...
            Parameters
            ----------
                table_name -> name of the TinyDB table to which a record
                must be updated
                record_id -> doc_id of the record that must be updated
                data_key -> name of the record's key that must be updated
                new_value -> new value that must be assigned to the record's
                key
                append -> a boolean that specifies if new_value must be
                appended (=True) to
        ...
        if append:
            key_value = \
                getattr(self, table_name).get(doc_id=record_id)[data_key]
            key_value.append(new_value)
            getattr(self, table_name).update({data_key: key_value},
                                             doc_ids=[record_id])
        else:
            getattr(self, table_name).update({data_key: new_value},
                                             doc_ids=[record_id])
        return
    ...
    def get_record_data(self, table_name, record_id, data_key=None):
        ...
        Retrieves a record of a table of a DBManager object
            ...
            ----------
                ...
                data_key -> name of the record's key that must be updated.
                If a data_key is
                entered, only the value of that key is retrieved,
                if no data_key is entered
                (=None), the entire record is retrieved

            Returns
            -------
                The entire record data (as a TinyDB table document) if
                data_key=None or
