...
class View:
    ...
    def show_welcome_message():
        print("Bienvenue dans votre logiciel de gestion de tournois d'échecs.")
        print("Merci de bien vouloir faire votre choix parmi le menu ci-dessous:")
...
class HomePage(View):
    ...
    def ask_user_choice(self):
        ...
        if choice not in self.menu or choice == "0":
            print("Choix non valide. Veuillez ressaisir un choix parmi la liste.")
            return HomePage()
        elif choice == "1":
...
class Tournaments(View):
    def show_menu(self):
        ...
        new_tournament = models.Tournaments(name=name,place=place)
        print(f"Le tournoi {new_tournament.name} se déroule à {new_tournament.place} en mode {new_tournament.time_control}.")
        input()
...
class Players(View):
    def show_menu(self):
        ...
        choice = input(">>> ")
        if choice == "1":
            players = sorted(models.Players.all_players,key=lambda x:x.family_name)
            if len(players) < 1:
                print("Aucun joueur n'est renseigné dans la base de données.")
            for player in players:
                print(player.first_name+" "+player.family_name+" - Classement -> "+player.ranking)
            input(">>> ")
        elif choice == "2":
            players = sorted(models.Players.all_players,key=lambda x:x.ranking)
            if len(players) < 1:
                print("Aucun joueur n'est renseigné dans la base de données.")
            for player in players:
                print(player.first_name+" "+player.family_name+" - Classement -> "+player.ranking)
...
class AddPlayers(View):
    def show_menu(self):
        ...
        new_player = models.Players(first_name=first_name,family_name=family_name,birth_date=birth_date,sex=sex,ranking=ranking)
        print(f"{new_player.first_name} {new_player.family_name} a été ajouté à notre base de joueurs.")
        input()
    ...
    def show_menu(self):
        ...
        for tournament in models.Tournaments.all_tournaments:
            if tournament.name == choice:
                print("Nombre de joueurs déjà inscrit au tournoi",choice,":",len(tournament.players))
...
class Matches(View):
    def ask_for_tournament(self):
        ...
        if tournament not in models.Tournaments.all_tournaments:
            ...
            for i,tournament in enumerate(models.Tournaments.all_tournaments,start=1):
                print(i,": ",tournament)
    ...
    def show_menu(self):
        ...
        if choice == "3":
            if len(models.Tournaments.all_tournaments) <1:
                print("Aucun tournoi n'est renseigné dans la base de données")
            else:
